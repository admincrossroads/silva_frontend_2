...
export default function QuarterlyReportsPage() {
  ...
  const { data: reports = [], isLoading } = useQuery<Report[]>({
    queryKey: ["reports", { type: "quarterly" }],
    queryFn: () => reportApi.findAll({ type: "quarterly" }),
  });

  const generate = useMutation({
    mutationFn: () => reportApi.generateQuarterly(),
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Requires Enhanced Governance. SPX authors narrative before Silva release.
          </p>
        </div>
        {canDraft && (
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 text-muted-foreground">No quarterly reports yet.</div>
      ) : (

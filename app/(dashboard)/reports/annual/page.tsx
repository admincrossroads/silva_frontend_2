...
export default function AnnualReportsPage() {
  ...
  const { data: reports = [], isLoading } = useQuery<Report[]>({
    queryKey: ["reports", { type: "annual" }],
    queryFn: () => reportApi.findAll({ type: "annual" }),
  });

  const generate = useMutation({
    mutationFn: () => reportApi.generateAnnual(),
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Year-end draft. SPX must author a narrative and explicitly release before Silva sees it.
          </p>
        </div>
        {canDraft && (
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 text-muted-foreground">No annual reports yet.</div>
      ) : (
